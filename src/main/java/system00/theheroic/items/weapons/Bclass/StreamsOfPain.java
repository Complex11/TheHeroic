...
public class StreamsOfPain extends ToolSword {
	...
	public void addInformation(ItemStack stack, @Nullable World world, List<String> tooltip, ITooltipFlag flag) {
		...
		tooltip.add("§d§lPassive: §r§7Normal attacks deal 2 damage and §3Drown §7targets. If the target is already §3Drowned§7, deal 4 extra damage and apply §2Absorption[2] §7to wielder. If the target's remaining health is less than 70% of it's total health, deal 6 extra damage and apply §2Speed[3] §7 to wielder.  If the target's remaining health is less than 40% of it's total health, deal 8 extra damage and apply §2Strength[3] §7and §2Water Breathing[1] §7to wielder.");
	...
	public boolean hitEntity(ItemStack stack, EntityLivingBase target, EntityLivingBase attacker) {
		...
		if (!target.getEntityData().getBoolean(NBT_KEY)) {
			...
			target.attackEntityFrom(DamageSource.DROWN, 2);
		} else if (target.getEntityData().getBoolean(NBT_KEY)) {
			target.attackEntityFrom(DamageSource.DROWN, 4);
