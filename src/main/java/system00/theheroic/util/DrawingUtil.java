...
public final class DrawingUtil {
	...
	/**
	 * Shorthand for {@link DrawingUtil#drawTexturedRect(int, int, int, int, int, int, int, int)} which draws the
	...
	public static void drawTexturedRect(int x, int y, int u, int v, int width, int height, int textureWidth, int textureHeight){
		DrawingUtil.drawTexturedFlippedRect(x, y, u, v, width, height, textureWidth, textureHeight, false, false);
	}
	...
	public static void drawScaledStringToWidth(FontRenderer font, String text, float x, float y, float scale, int colour, float width, boolean centre, boolean alignR){
		...
		DrawingUtil.drawScaledTranslucentString(font, text, x, y, scale, colour);
	}
